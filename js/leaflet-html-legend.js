...
L.Control.HtmlLegend = L.Control.extend({
    ...
    _renderLegend(legend) {
        ...
        if (legend.name) {
            const header = L.DomUtil.create('h4', "card-header bg-dark-primary", block);
        ...
        //elementContainer.style.margin = "6px 0px 0px 0px"

        const removeButton = L.DomUtil.create('button', `btn btn-danger legend-button`, elementContainer);
        removeButton.innerHTML = "Remove"
        L.DomEvent.on(removeButton, 'click', (e) => {
            ipcRenderer.send('removeTrack', legend.name);
            legend.layer._map.removeLayer(legend.layer)
        });

        const zoomButton = L.DomUtil.create('button', `btn btn-info legend-button`, elementContainer);
        zoomButton.id = "legend-" + legend.name;
        zoomButton.innerHTML = "Render"
        L.DomEvent.on(zoomButton, 'click', (e) => {
            if ($("#v-pills-tab a.active")[0].id != "v-pills-home-tab") return;
            legend.layer._map.flyToBounds(legend.layer.getBounds(), {
                duration: 1.25
            });
        });
